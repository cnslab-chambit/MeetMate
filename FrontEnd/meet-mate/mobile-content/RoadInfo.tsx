...
import { BaseBar, RoadInfoDiv, WayBar } from "@/m-styled-component/road-compontnt/road_styled";
...
function RoadInfo(pathRecoil: any) {
    ...
    return(
        ...
        <TransferBox>
                <TimeBox>
                    <FlexBox>
                        <Boldpg>{Math.floor(pathRecoil.pathRecoil?.info?.totalTime / 60)}</Boldpg>시간
                    ...
                    <FlexBoxCol>
                        <BaseBar>
                        {
                            pathRecoil.pathRecoil?.subPath.map((subPath: any, index: number) => 
                            <Gauge 
                            key={index}
