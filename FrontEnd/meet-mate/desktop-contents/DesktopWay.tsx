...
import WalkingIcon from "../public/images/Walking.svg";
...
const printName = (type: any, props: any) => {
    if (type === 3) {
        return `도보 - ${props?.sectionTime}분`;
    }
    else if (type === 1) {
        return `${props?.subwayName} - ${props?.sectionTime}분`;
    }
    else if (type === 2) {
        return `${props?.busNo} - ${props?.sectionTime}분`;
    }
}

function DesktopWay(props: any) {
    ...
    useEffect(() => {
        if (trafficType === 3) {
            ...
            //bus -> type 11이면 마을버스?
            if (props.buswayCode === 4 || props.buswayCode === 6 || props.buswayCode === 14) {
                setColor("#E86359");
        ...
        <WayContainer>
            ...
            >
                <svg viewBox="0 0 50 50">
                    {trafficType === 1 ? <SubwayIcon /> : trafficType === 2 ? <BusIcon /> : <WalkingIcon />}
                </svg>
            </IconDiv>
            {printName(trafficType, props)}
        </WayContainer>
    )
}

export default DesktopWay;