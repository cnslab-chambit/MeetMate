...
import Gauge from "../app/_component/Gauge";
import Way from "../app/_component/Way";
import { subwayPathState } from "../../_atom/atom";

function SubWay() {
  const [roadData, setRoadData] = useRecoilState(subwayDataState);
  const [roadSearchResult, setRoadSearchResult] = useRecoilState(
    roadSearchResultState
  );
  ...
  return (
    <TransferBox>
      {roadData.path.length > 0
        ? roadData?.path.map((path: any, index: number) => {
            if (1 === path.pathType) {
              return (
                <TimeBox key={index} onClick={() => onWay(path)}>
                  <FlexBox>
                    ...
                    <FlexBox>요금 {path.info.payment}원</FlexBox>
                  </FlexBox>
                      ...
                      {roadData.path.length > 0
                        ? path?.subPath.map((subPath: any, index: number) => (
                            <Gauge
                              ...
                              lane={
                                subPath?.lane ? subPath.lane[0].name : "도보"
                              }
                              subwayCode={
                                subPath?.lane
