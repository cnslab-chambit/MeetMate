...
function SubwayDot() {
  ...
  const dotGraph = () => {
    return (
      <SubStationDiv>
        {subPath.map((e: any, index: number) => {
          ...
          return (
            <SubwayViewDetailDiv>
              ...
              <SubwayViewDiv>
                {e?.passStopList?.stations.map((e: any, index: number) => {
                  return (
                    <div key={index}>
                      <StationDiv active={currentColor}>
                        <DotIcon />
                        <StationNameText>{e?.stationName}</StationNameText>
                      </StationDiv>
                      <ArrowDiv>
              ...
              {subPath.length - 1 === index ? null : <DownIcon />}
            </SubwayViewDetailDiv>
          );
        })}
      </SubStationDiv>
